...
export default function CarPricingPage() {
  ...
  const [rawPriceData, setPriceData] = useState<PriceData[]>([]);
  ...
  useEffect(() => {
    async function fetchHeatmapData() {
      try {
        const res = await fetch("/api/heatmapDatas");
        const rawData: { car: string; demand: { region: string; value: number }[] }[] = await res.json();

        // Convert raw API data to HeatMapSeries format
        // const convertedData: HeatMapSeries[] = rawData.map((item) => ({
        //   id: item.car,
        //   data: item.demand
        //     .filter(d => d.value !== null && d.value !== undefined && !isNaN(d.value))
        //     .map(d => ({ x: d.region, y: Number(d.value) })),
        // }));
        ...
        const convertedData: HeatMapSeries[] = rawData.map((item) => ({
          ...
          data: item.demand
            .filter(d => d.value !== null && d.value !== undefined && !isNaN(d.value))
            .map(d => ({
              x: String(d.region), // ensure string
              y: Number(d.value)   // ensure number
            })),
        }));
...
interface HeatMapSeries {
  ...
  const glassClass =
    ...
    // const transformedHeatmapData = heatmapData.map((item) => ({
    //   id: item.id,
    //   data: item.data,
    // }));
      ...
      <div className="pt-30 px-6 md:px-12 lg:px-24 pb-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          ...
          <div className={glassClass}>
            ...
            <h3 className="text-2xl font-bold text-blue-600 dark:text-emerald-400">
              ${Math.max(...rawPriceData.map((d) => d.actual)).toLocaleString()}
            </h3>
          </div>
        </div>
          ...
          <div className={glassClass}>
            ...
            <div className="h-120">
              <ResponsiveBar
                ...
                indexBy="car"
                margin={{ top: 20, right: 20, bottom: 200, left: 70 }}
                padding={0.3}
                colors={chartColors}
                axisBottom={{
                  legend: "Car",
